...
const getMainAssetWithinDepartment = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      // if (util.empty(req.query.transferFormNo))
      //   return res.status(400).json({ error: "`Transfer Form Number` is required." });
      ...
      const assetCode = req.query.assetCodeResult;
      const depts = req.query.receivingDepartment;

      let sqlWhere = "";
      // const userDepartmentCode = util.currentUserToken(req).deptCode
      let args = [];
      sqlWhere = ` and  oldAssetCode = ? and oldAssetCode IS not NULL and  oldAssetCode != ''  and receivingDepartment =?`;
      args = [assetCode, depts];
      const options = {
        top: "",
        order: "",
      };
...
//[AsventoSubDepartments]
const getAllDeptCodes = async function (req, txn) {
  const userDepartmentCode = util.currentUserToken(req).deptCode;

  const sqlWhere = `AND subDept.primaryDeptCode = ?`;
  const args = [userDepartmentCode];
  const options = {
    top: "",
    order: "",
  };

  const result = await departments.primaryDept(sqlWhere, args, options, txn);

  if (result.error !== undefined) {
    throw new Error(result.error);
  }

  // Assuming result is an array of objects with subDeptCode
  return result.map((row) => row.allDepts);
};
...
//   const returnValue = await sqlHelper.transact(async (txn) => {
//     const deptCodesArray = await getAllDeptCodes(req, txn);

//     // Validate the array and content
//     if (
//       !deptCodesArray ||
//       deptCodesArray.length === 0 ||
//       !deptCodesArray[0] ||
//       deptCodesArray[0].trim() === ""
//     ) {
//       return []; // Return empty if no valid departments
//     }

//     // Split the single string into individual codes
//     const deptCodes = deptCodesArray[0]
//       .split(",")
//       .map((code) => code.trim())
//       .filter((code) => code !== "");

//     const deptCodesString = deptCodes.map((code) => `'${code}'`).join(",");

//     // Final SQL (raw string)
//     const sqlWhere = `AND active = 1 AND receivingDepartment IN (${deptCodesString}) AND categoryId <> 30`;
...
//     return await assets.selectAssets(sqlWhere, [], options, txn);
//   });
...
// const getAssetsByDepartment = async function (req, res) {
//   const returnValue = await sqlHelper.transact(async (txn) => {
//     // Get all sub-department codes for logged-in user
//     const deptCodes = await getAllDeptCodes(req, txn);
...
//     // Dynamically build WHERE IN placeholders
//     const placeholders = deptCodes.map(() => "?").join(", ");

//     // Final WHERE clause
//     const sqlWhere = `AND active = ? AND receivingDepartment IN (${placeholders}) `;
//     const args = [1, ...deptCodes];
...
// const getAssetsByDepartment = async function (req, res) {
//   const returnValue = await sqlHelper.transact(async (txn) => {
//     // Get the department codes as an array with a single string
//     const deptCodesArray = await getAllDeptCodes(req, txn); // Example: ['5026, 5013, 5050']

//     if (!deptCodesArray || deptCodesArray.length === 0) {
//       return [];
//     }

//     // Split the single string into individual codes
//     const deptCodes = deptCodesArray[0].split(",").map((code) => code.trim());

//     // Format: '5026','5013','5050'
//     const formattedDeptCodes = deptCodes.map((code) => `'${code}'`).join(",");

//     // Final SQL (raw string)
//     const sqlWhere = `AND active = 1 AND receivingDepartment IN (${formattedDeptCodes}) `;
...
const getByDeptAssetPendingCondemn = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    const userDepartmentCode = util.currentUserToken(req).deptCode;
    let args = [];
    sqlWhere = `and latest.rn = ? and latest.RequestedDepartment=?`;
...
const getAllAssetCondemnRequest = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    sqlWhere = `and latest.rn = ?`;
    args = [1];
    const options = {
      top: "",
      order: "CondemRequestedDate desc",
    };
    return await assets.distinctPendingCondemByDepartmentProperty(
      sqlWhere,
      args,
      options,
      txn,
...
const getAllAssetCondemnRequestForAccounting = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    let sqlWhere = "";
    const condemnReStatus = "Pending";
    // const userDepartmentCode = util.currentUserToken(req).deptCode
    ...
    sqlWhere = `and AraForm is not null and AraForm like  '%ARA%' and CondemnReStatus= ? and CondemnStatus = ? `;
...
const getAssetsActive = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    let sqlWhere = "";
    let args = [];
    sqlWhere = `and categoryId <> ?`; //open to all even inactive
    args = ["30"];
    const options = {
      top: "",
      order: "DateTimeUpdated desc ",
...
const getARAPartsDetails = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      if (util.empty(req.query.araForm))
        return res
...
const getUpcomingPartsCondemRequest = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      ...
      // const userDepartmentCode = util.currentUserToken(req).deptCode;
      let args = [];
      sqlWhere = ` and condemnReStatus=? and araForm IS NOT NULL AND araForm <> '' and administrator =?`;
      args = [condemnReStatus, administrator];
    ...
    for (const asset of assetsInfo) {
      try {
        assetStatus = await sqlHelper.transact(async (txn) => {
          ...
          if (!insertAssetStatus) {
            throw new Error("Failed to insert asset status");
          }
    ...
    for (const parts of partsInclude) {
      try {
        assetStatus = await sqlHelper.transact(async (txn) => {
          ...
          if (!insertAssetStatus) {
            throw new Error("Failed to insert part status");
          }
        });
      } catch (error) {
        return res.status(500).json({ error: error.message });
...
// const deptViewAccess = async function (req, res) {
//   try {
//     if (util.empty(req.body))
//       return res.status(400).json({ error: "`body` is required." });
//     const { primary, subsDept } = req.body;
...
//       const assetsPayload = {
//         code: generatedCode,
//         primaryDeptCode: primary,
//         subDeptCode: subsDept,
//       };

//       const insertAssetStatus = await assets.updateDeptViewAccess(assetsPayload, txn);
...
const deptViewAccess = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    try {
      ...
      const combinedSubDept = subsDept.join(",");
      updateAssetInfo = await assets.updateDeptViewAccess(
        {
          createdBy: activeUser,
          subDeptCode: combinedSubDept,
        },
        { primaryDeptCode: primary },
        txn,
      );
...
const auditAssetUpdate = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    const { assetsInfo, partsAssigned } = req.body;
    ...
    try {
      ...
      // UPDATE PARTS INFO START//
      for (const partsDets of partsAssigned) {
        // UPDATE PARTS INFO START//
        await assetsComponents.updateAssetsComponents(
          {
            location: assetsInfo.location,
          },
          { code: partsDets.componentCode },
          txn,
        );
        //UPDATE PARTS INFO END//
      }
        ...
        for (const asset of assetsInfo) {
          ...
          if (assetComponents.length > 0) {
            ...
            for (const parts of assetComponents) {
              ...
              if (insertAssetStatus.error) {
                console.error(insertAssetStatus.error); // Log error details
                // throw new Error(insertAssetStatus.error); // Bubble up the error
              }
  ...
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    try {
      ...
      if (existingAssets && existingAssets.length > 0) {
        ...
        const mainlocation = oldAsset.location;
...
//IT APPROVAL TRANSFER REQUEST PARTS
const putTransferApproveParts = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    const { assetsInfo, partsTransferInfo, assetCode } = req.body;
    const activeUser = util.currentUserToken(req).code;
    const assetCodeMain = assetCode.transferingAssetCode;
    ...
    try {
      ...
      if (assetCodeMain) {
        ...
        if (existingAssets && existingAssets.length > 0) {
          const oldAsset = existingAssets[0];
          oldAssetCode = oldAsset.code;
          deptPartment = oldAsset.receivingDepartment;
          mainlocation = oldAsset.location;
        }
      } else {
        // return res.status(404).json({ error: "Asset code not found TEST." });

        oldAssetCode = null;
        deptPartment = assetsInfo[0].transferredDepartment;
...
const updateAssetPhysicalLocation = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    const { assetsInfo, includedParts, newLocation, accountablePerson } =
      req.body;
    try {
      let updateAssetInfo = "";
      updateAssetInfo = await assets.updateAssets(
        {
          ...
          updatedBy: util.currentUserToken(req).code,
          accountableEmployee: accountablePerson,
        },
        { code: assetsInfo.code },
        txn,
      );
...
module.exports = {
  ...
  getARAPartsDetails,
  getMainAssetWithinDepartment,
  putTransferApproveParts,
  getAllDeptCodes,
  deptViewAccess,
  getAllAssetCondemnRequestForAccounting,
};
