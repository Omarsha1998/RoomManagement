...
const distinctPendingCondemByDepartment = async function (
  ...
  try {
    return await sqlHelper.query(
      ...
      ${util.empty(options.top) ? "" : `TOP(${options.top})`}
       latest.AraForm,
    latest.RequestedDepartment,
    latest.CondemRequestedDate
    FROM (
    SELECT 
        asst.AraForm,
        retirementStatus.RequestedDepartment,
        retirementStatus.CondemRequestedDate,
        ROW_NUMBER() OVER (
            PARTITION BY asst.AraForm 
            ORDER BY retirementStatus.CondemRequestedDate DESC
        ) AS rn
    FROM UERMINV..Assets asst
    JOIN [UERMINV].[dbo].[CondemnationHistory] retirementStatus
        ON asst.code = retirementStatus.internalAssetCode
    WHERE asst.condemnReStatus = 'Pending'
) latest
		where 1=1 ${conditions}
...
const distinctPendingCondemByDepartmentProperty = async function (
  conditions,
  args,
  options,
  txn,
...
module.exports = {
  ...
  updateDeptViewAccess,
  distinctPendingCondemByDepartmentProperty,
};
