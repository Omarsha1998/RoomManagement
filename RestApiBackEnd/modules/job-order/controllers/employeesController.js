...
const getActiveEmployeesBiomed = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      // if (util.empty(req.query.transferFormNo))
      //   return res.status(400).json({ error: "`Transfer Form Number` is required." });
      ...
      // const userDepartmentCode = util.currentUserToken(req).deptCode
      let args = [];
      sqlWhere = ` and is_active = ? and DEPT_CODE = ?`;
      args = [1, "1880"];
      const options = {
        top: "",
        order: "",
...
const getITEmployees = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      // if (util.empty(req.query.transferFormNo))
      //   return res.status(400).json({ error: "`Transfer Form Number` is required." });
...
const getEmployeeDeptGroup = async function (req, res) {
  try {
    const returnValue = await sqlHelper.transact(async (txn) => {
      // if (util.empty(req.query.transferFormNo))
      //   return res.status(400).json({ error: "`Transfer Form Number` is required." });
...
module.exports = {
  ...
  getActiveEmployeesBiomed,
  getITEmployees,
  getEmployeeDeptGroup,
};
